...
class GamificationController:
    ...
    @staticmethod
    def _create_default_quests(user_id: str) -> list[dict]:
        """Constructs the default sustainability quest list."""
        return [
            {
    ...
    @staticmethod
    def _format_challenges(challenges: list) -> list[dict]:
        """Formats challenge documents for JSON API response validation."""
    ...
    async def get_challenges(db: Any, current_user: dict) -> list:
        ...
        if not challenges:
            default_quests = GamificationController._create_default_quests(user_id)
            for quest in default_quests:
                quest_id = await repo.insert_challenge(quest)
                quest["_id"] = quest_id
            challenges = default_quests

        return GamificationController._format_challenges(challenges)

    @staticmethod
    def _validate_challenge_for_claim(challenge: dict, user_id: str) -> None:
        """Verifies if the user is authorized and has completed the challenge to claim reward."""
    ...
    async def claim_challenge(challenge_id: str, db: Any, current_user: dict) -> dict:
        ...
        GamificationController._validate_challenge_for_claim(challenge, user_id)
